...
async def ban_user(chat_id, i, rights):
    try:
        await jmub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@jmub.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@jmub.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@jmub.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await jmub(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in jmub.iter_participants(chat_id):
        ...
        await jmub.send_message(chat_id, usrtxt)
...
@jmub.ar_cmd(pattern="الغاء التفليش", groups_only=True)
