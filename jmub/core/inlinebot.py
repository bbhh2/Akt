...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**jmub"):
            ...
            builder = event.builder
            troll = os.path.join("./jmub", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./jmub", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./jmub", "hide.txt")
            try:
            ...
            result = builder.article(
                title="jmub Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب أكثون **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/VV744"),
                Button.url(
                    "المطور",
                    "t.me/MIMIM5",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل أكثون من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝘼𝙆𝙏𝙃𝙊𝙉 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](t.me/VV744)",
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
