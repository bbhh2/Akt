...
ROE = """** اهلا بك عزيزي المستخدم في قائمة اوامر أكثون
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == jmub.uid:
            buttons = [
                [Button.inline("معلومات أكثون", data="AOMRDB")],
                [
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(rb"chekuserq")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="CEHKUSERNAME")]]
...
async def varssett(event):
    await event.edit(
        """اهلا بك في قائمة اوامر سورس أكثون هذه بعض المعلومات عن أكثون:

- سورس أكثون يمتلك اكثر من 100 أمر 
...
@jmub.tgbot.on(CallbackQuery(data=re.compile(rb"Deltejm")))
@check_owner
async def _(event):
